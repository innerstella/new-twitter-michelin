...
트위터에서 언급된 맛집을 검색할 수 있는 서비스. 소상공인 가게 찾기 기능도 포함.

### 기술 스택

- **Frontend**: React 18, TypeScript, CRA (craco)
- **Styling**: styled-components, Radix UI Themes, Chakra UI
- **상태관리**: Zustand
- **애니메이션**: Framer Motion
...
├── pages/
│   ├── home/          # 메인 검색 페이지
│   ├── small-shop/    # 소상공인 가게 찾기 (Supabase)
│   ├── hotpot/        # 훠궈 소스 백과사전 (Firebase)
│   ├── recommend/     # 추천 해시태그
│   ├── group/         # 그룹별 보기
│   ├── group-list/    # 그룹 목록
│   └── log/           # 변경 로그
├── store/             # Zustand 스토어
├── lib/               # supabase.ts 등 클라이언트 설정
├── model/             # 타입 및 상수
├── components/        # 공통 컴포넌트
...
```bash
npm start        # 개발 서버
npm run build    # 프로덕션 빌드 (react-snap 자동 실행)
